...
// Generated 06-Jul-2021 14:52:33
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"micromodelswil\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(micromodelswil_M, (NULL));
        micromodelswil_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(micromodelswil_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(micromodelswil_M)
                == (NULL)) || rtmGetStopRequested(micromodelswil_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(micromodelswil_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
